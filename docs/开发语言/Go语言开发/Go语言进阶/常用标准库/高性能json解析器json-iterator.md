...
![图片](./assets/高性能json解析器json-iterator/1.png)
...
> **经多次运行，发现反序列化(`Unmarshal`)比标准包快了2.5倍,而序列化(`Marshal`)基本没有什么很高的性能提升。**
...
**@设置`SetNamingStrategy`后，有`json`标签的使用`json`标签，没有的则会自动转成xx_xx**

## 4. 避坑:`concurrent map writes`

**@设置`extra.RegisterFuzzyDecoders()`时，注意不要在多个协程中重复设置，在init函数执行一次即可，否则会报错：fatal error: concurrent map writes**
