...
![Linux TCP 参数](./assets/如何优化TCP/1.jpg)
...
![三次握手与数据传输](./assets/如何优化TCP/3.jpg)
...
![TCP 三次握手的状态变迁](./assets/如何优化TCP/4.jpg)
...
![半连接队列与全连接队列](./assets/如何优化TCP/8.jpg)
...
![img](./assets/如何优化TCP/10.jpg)

增大 backlog 的方式，每个 Web 服务都不同，比如 Nginx 增大 backlog 的方法如下：

![img](./assets/如何优化TCP/11.jpg)
...
![开启 syncookies 功能](./assets/如何优化TCP/12.jpg)
...
![tcp_abort_on_overflow 为 0 可以应对突发流量](./assets/如何优化TCP/17.jpg)
...
![常规 HTTP 请求](./assets/如何优化TCP/20.jpg)

在 Linux 3.7 内核版本之后，提供了 TCP Fast Open 功能，这个功能可以减少 TCP 连接建立的时延。

> 接下来说说，TCP Fast Open 功能的工作方式。

![开启 TCP Fast Open 功能](./assets/如何优化TCP/21.jpg)
...
![TCP option 字段 - TFO](./assets/如何优化TCP/22.png)
...
![三次握手优化策略](./assets/如何优化TCP/24.jpg)
...
![img](./assets/如何优化TCP/27.jpg)

你可能会好奇，这 0 表示几次？ **实际上当为 0 时，特指 8 次** ，从下面的内核源码可知：

![img](./assets/如何优化TCP/28.jpg)
...
![TIME-WAIT 时间过短，收到旧连接的数据报文](./assets/如何优化TCP/31.png)
...
![TIME-WAIT 时间过短，没有确保连接正常关闭](./assets/如何优化TCP/32.png)

服务端收到这个 RST 并将其解释为一个错误（Connection reset by peer），这对于一个可靠的协议来说不是一个优雅的终止方式。

为了防止这种情况出现，客户端必须等待足够长的时间，确保服务端能够收到 ACK，如果服务端没有收到 ACK，那么就会触发 TCP 重传机制，服务端会重新发送一个 FIN，这样一去一来刚好两个 MSL 的时间。

![TIME-WAIT 时间正常，确保了连接正常关闭](./assets/如何优化TCP/33.png)
...
![TCP option 字段 - 时间戳](./assets/如何优化TCP/37.png)
...
![img](./assets/如何优化TCP/38.jpg)

> TIME_WAIT 状态优化方式三

我们可以在程序中设置 socket 选项，来设置调用 close 关闭连接行为。

![img](./assets/如何优化TCP/39.jpg)
...
![四次挥手的优化策略](./assets/如何优化TCP/41.jpg)
...
![按数据包进行确认应答](./assets/如何优化TCP/42.jpg)

所以，这样的传输方式有一个缺点：数据包的 **往返时间越长，通信的效率就越低** 。

 **要解决这一问题不难，并行批量发送报文，再批量确认报文即可。**  

![并行处理](./assets/如何优化TCP/43.jpg)
...
![TCP option 选项 - 窗口扩展](./assets/如何优化TCP/45.png)

Linux 中打开这一功能，需要把 tcp_window_scaling 配置设为 1（默认打开）：

![img](./assets/如何优化TCP/46.jpg)
...
![img](./assets/如何优化TCP/50.jpg)

> 调节 TCP 内存范围

接收缓冲区调节时，怎么知道当前内存是否紧张或充分呢？这是通过 tcp_mem 配置完成的：

![img](./assets/如何优化TCP/51.jpg)
...
![数据传输的优化策略](./assets/如何优化TCP/52.jpg)
