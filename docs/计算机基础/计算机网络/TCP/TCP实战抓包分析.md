...
![tcpdump 常用选项类](./assets/TCP实战抓包分析/6.jpg)
...
![tcpdump 常用过滤表达式类](./assets/TCP实战抓包分析/7.jpg)
...
![ping 网络包](./assets/TCP实战抓包分析/10.jpg)
...
![TCP 流量图](./assets/TCP实战抓包分析/15.jpg)
...
![取消序列号相对值显示](./assets/TCP实战抓包分析/16.jpg)

取消后，Seq 显示的就是真实值了：

![TCP 流量图](./assets/TCP实战抓包分析/17.jpg)

可见，客户端和服务端的序列号实际上是不同的，序列号是一个随机值。

这其实跟我们书上看到的 TCP 三次握手和四次挥手很类似，作为对比，你通常看到的 TCP 三次握手和四次挥手的流程，基本是这样的：

![TCP 三次握手和四次挥手的流程](./assets/TCP实战抓包分析/18.jpg)
...
![img](./assets/TCP实战抓包分析/20.jpg)

其间 tcpdump 抓包的命令如下：

![img](./assets/TCP实战抓包分析/21.jpg)

过了一会， curl 返回了超时连接的错误：

![img](./assets/TCP实战抓包分析/22.jpg)

从 `date` 返回的时间，可以发现在超时接近 1 分钟的时间后，curl 返回了错误。

接着，把 tcp_sys_timeout.pcap 文件用 Wireshark 打开分析，显示如下图：

![SYN 超时重传五次](./assets/TCP实战抓包分析/23.jpg)
...
![SYN 超时重传两次](./assets/TCP实战抓包分析/24.jpg)
...
![常规 HTTP 请求](./assets/TCP实战抓包分析/42.jpg)

在 Linux 3.7 内核版本中，提供了 TCP Fast Open 功能，这个功能可以减少 TCP 连接建立的时延。

![常规 HTTP 请求 与 Fast  Open HTTP 请求](./assets/TCP实战抓包分析/43.jpg)
...
![TCP Fast Open 抓包分析](./assets/TCP实战抓包分析/44.jpg)
...
![快速重传机制](./assets/TCP实战抓包分析/45.jpg)

TCP 重复确认和快速重传的一个案例，用 Wireshark 分析，显示如下：

![img](./assets/TCP实战抓包分析/46.jpg)
...
![理想状态下的窗口变化](./assets/TCP实战抓包分析/47.jpg)
...
![服务端繁忙状态下的窗口变化](./assets/TCP实战抓包分析/48.jpg)
...
![窗口大小在收缩](./assets/TCP实战抓包分析/49.jpg)

接着，发送方会 **定时发送窗口大小探测报文** ，以便及时知道接收方窗口大小的变化。

以下图 Wireshark 分析图作为例子说明：

![零窗口 与 窗口探测](./assets/TCP实战抓包分析/50.jpg)
...
![禁用 Nagle 算法 与 启用 Nagle 算法](./assets/TCP实战抓包分析/53.jpg)
...
![关闭 Nagle 算法](./assets/TCP实战抓包分析/54.jpg)
...
![关闭 TCP 延迟确认](./assets/TCP实战抓包分析/58.jpg)

> 延迟确认 和 Nagle 算法混合使用时，会产生新的问题

当 TCP 延迟确认 和 Nagle 算法混合使用时，会导致时耗增长，如下图：

![TCP 延迟确认 和 Nagle 算法混合使用](./assets/TCP实战抓包分析/59.jpg)
