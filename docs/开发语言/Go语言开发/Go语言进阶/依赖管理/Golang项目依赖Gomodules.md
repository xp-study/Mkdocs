...
- 七牛云
  ...
  [https://goproxy.cn](https://goproxy.cn/),direct
...
设置后，前缀为 [git.xxx.com](http://git.xxx.com/) 和 [github.com/eddycjy/mquote](http://github.com/eddycjy/mquote) 的模块都会被认为是私有模块。
...
这样子设置的话，所有模块路径为 [example.com](http://example.com/) 的子域名（例如：[git.example.com](http://git.example.com/)）都将不经过 Go module proxy 和 Go checksum database，**需要注意的是不包括 [example.com](http://example.com/) 本身**。
