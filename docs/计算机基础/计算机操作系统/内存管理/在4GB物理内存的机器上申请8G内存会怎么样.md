...
- 如果没有空闲的物理内存，那么内核就会开始进行 **回收内存 (opens new window)** 的工作，如果回收内存工作结束后，空闲的物理内存仍然无法满足此次物理内存的申请，那么内核就会放最后的大招了触发 OOM （Out of Memory）机制。

32 位操作系统和 64 位操作系统的虚拟地址空间大小是不同的，在 Linux 操作系统中，虚拟地址空间的内部又被分为 **内核空间和用户空间** 两部分，如下所示：

![img](./assets/在4GB物理内存的机器上申请8G内存会怎么样/1.png)
...
![img](./assets/在4GB物理内存的机器上申请8G内存会怎么样/2.png)
...
![img](./assets/在4GB物理内存的机器上申请8G内存会怎么样/3.png)
...
![img](./assets/在4GB物理内存的机器上申请8G内存会怎么样/4.png)
...
![img](./assets/在4GB物理内存的机器上申请8G内存会怎么样/5.png)
...
![img](./assets/在4GB物理内存的机器上申请8G内存会怎么样/6.png)
...
![img](./assets/在4GB物理内存的机器上申请8G内存会怎么样/7.png)
...
![img](./assets/在4GB物理内存的机器上申请8G内存会怎么样/8.png)
...
![img](./assets/在4GB物理内存的机器上申请8G内存会怎么样/9.png)
...
![img](./assets/在4GB物理内存的机器上申请8G内存会怎么样/10.png)

此时我的笔记本电脑的磁盘开始出现“沙沙”的声音，通过查看磁盘的 I/O 情况，可以看到磁盘 I/O 达到了一个峰值，非常高：

![img](./assets/在4GB物理内存的机器上申请8G内存会怎么样/11.png)
...
![img](./assets/在4GB物理内存的机器上申请8G内存会怎么样/12.png)
